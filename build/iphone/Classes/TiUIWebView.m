...
static NSString * const kTi_TeamTaskManagerJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
-(NSString*)_ti_teamtaskmanagerInjection
{
	...
	[html appendString:kTi_TeamTaskManagerJavascript];
...
{
	// attempt to make well-formed HTML and inject in our Ti_TeamTaskManager bridge code
	...
	if (range.location!=NSNotFound)
	{
		...
		if (nextRange.location!=NSNotFound)
		{
			...
			[html appendString:[self _ti_teamtaskmanagerInjection]];
	...
	else
	{
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:path])
		{
			...
			else
			{
				...
				{
					NSLog(@"[WARN] I have no idea what the appropriate text encoding is for: %@. Please report this to Ti_TeamTaskManager support.",url);
				}
			}
