...
@implementation Ti_TeamTaskManagerObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_ti_teamtaskmanager);
	RELEASE_TO_NIL(modules);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
-(void)injectPatches
{
	// called to inject any Ti_TeamTaskManager patches in JS before a context is loaded... nice for 
...
{
	// create Ti_TeamTaskManager global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_ti_teamtaskmanager = [[Ti_TeamTaskManagerObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_ti_teamtaskmanager];
	
	NSString *_ti_teamtaskmanagerNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _ti_teamtaskmanagerNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_ti_teamtaskmanager valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.ti_teamtaskmanager.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
